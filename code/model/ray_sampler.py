...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model, shape_code):
        beta0 = model.density.get_beta().detach()

        # Start with uniform sampling, z_vals.shape: N_samples_eval
        z_vals = self.uniform_sampler.get_z_vals(ray_dirs, cam_loc, model)
        assert torch.isnan(z_vals).sum() == 0, print('sample zvals_uniform met a nan')
        samples, samples_idx = z_vals, None

        # Get maximum beta from the upper bound (Lemma 2)
        dists = z_vals[:, 1:] - z_vals[:, :-1]
        bound = (1.0 / (4.0 * torch.log(torch.tensor(self.eps + 1.0)))) * (dists ** 2.).sum(-1)
        assert torch.isnan(bound).sum() == 0, print('sample bound met a nan')
        beta = torch.sqrt(bound)
        assert torch.isnan(beta).sum() == 0, print('sample sqrt met a nan')
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                samples_sdf = model.get_sdf_vals(points_flat, shape_code)
            ...
            assert torch.isnan(dists).sum() == 0, print('dists')
            ...
            if not_converge and total_iters < self.max_total_iters:
                ...
                assert torch.isnan(error_per_section).sum()==0, print(total_iters, torch.isnan(dists).sum())
                pdf = bound_opacity + self.add_tiny
                pdf = pdf / torch.sum(pdf, -1, keepdim=True)
                tmp = torch.sum(pdf, -1, keepdim=True)
                assert torch.isnan(pdf).sum()==0, print(total_iters, tmp == torch.zeros_like(tmp))
        ...
        idx = torch.randint(z_samples.shape[-1], (z_samples.shape[0],)).cuda()
        z_samples_eik = torch.gather(z_samples, 1, idx.unsqueeze(-1))
        # find a nearest point from surface
        # points = cam_loc.unsqueeze(1) + z_vals.unsqueeze(2) * ray_dirs.unsqueeze(1)
        # points_flat = points.reshape(-1, 3)
        # with torch.no_grad():
        #     pre_sdf = model.deform_temp_geo_network.get_sdf_vals(points_flat, shape_code)
        #     pre_sdf[pre_sdf<0] = 1e10
        #     pre_sdf = pre_sdf.reshape(points.shape[0],points.shape[1],1)
        # z_samples_eik_idx = torch.argmin(pre_sdf, axis=1)
        # # z_samples_eik = z_vals.view(1,-1)[:,z_samples_eik_idx].unsqueeze(-1)
        # z_samples_eik = z_vals.gather(1,z_samples_eik_idx)
        # print(z_samples_eik,pre_sdf.squeeze(-1).gather(1,z_samples_eik_idx))
        ...
        return z_vals, z_samples_eik, z_samples
    ...
    def get_error_bound(self, beta, model, sdf, z_vals, dists, d_star):
        ...
        assert torch.isnan(shifted_free_energy).sum() == 0, print('in error bound, shiterengy')
        integral_estimation = torch.cumsum(shifted_free_energy, dim=-1)
        assert torch.isnan(integral_estimation).sum() == 0, print('in error bound, before error per section')
        error_per_section = torch.exp(-d_star / beta) * (dists ** 2.) / (4 * beta ** 2)
        assert torch.isnan(error_per_section).sum() == 0, print('in error bound',beta)
        error_integral = torch.cumsum(error_per_section, dim=-1)

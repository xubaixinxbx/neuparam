...
def implicit2mesh(decoder, mods, grid_res, translate=[0., 0., 0.], scale=1, get_mesh=True, device=None,
                  bbox=np.array([[-1, 1], [-1, 1], [-1, 1]]), feat=None, hash_tree=None, return_sdf=False):
    ...
    if return_sdf:
        sdf_field = extract_fields(bound_min=bbox[:,0], bound_max=bbox[:,1], resolution=grid_res, query_func=lambda x: decoder(x.type(torch.float32), mods).detach().squeeze(0).cpu().numpy(), device=device)
    else:
        sdf_field = None
    # import pdb; pdb.set_trace()
    return mesh_list[0], sdf_field
        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    pts = torch.cat([xx.reshape(-1, 1), yy.reshape(-1, 1), zz.reshape(-1, 1)], dim=-1).float().to(device)

...
import skimage
from skimage import color as rtl
...
def load_rgb(path, normalize_rgb = False, resize_size = (128, 128), resize_flag=False, white_flag=False):
    # img = imageio.imread(path)
    img = np.array(Image.open(path))
    if white_flag:
        img = rtl.rgba2rgb(img)
    img = skimage.img_as_float32(img) # [0.0, 1.0]
...
def get_rays(uv, c2w, K):
    ...
    rays_o = c2w[:3,-1]
    uv = uv.long()
    rays_d = -1.0*rays_d[uv[:,0],uv[:,1],:]
    rays_d = F.normalize(rays_d, dim=-1)
    return rays_d.unsqueeze(0), rays_o.unsqueeze(0)
